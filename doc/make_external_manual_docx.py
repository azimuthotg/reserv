...
para("เวอร์ชัน 1.1  |  กรกฎาคม 2569", size=13, color=GRAY, align=WD_ALIGN_PARAGRAPH.CENTER)
...
simple_table(
    ...
    [
        ...
        ["ต้องกรอกเลขบัตรประชาชนไหม", "ไม่บังคับ (บังคับแค่ชื่อ-สกุล)", "ใช้เป็นตัวระบุตัวตน (VIP เว้นว่างได้)"],
...
numbered_list([
    ...
    "กรอก ชื่อ-นามสกุล (บังคับ) — เลขบัตรประชาชน 13 หลัก กรอกหรือไม่ก็ได้ (ไม่บังคับ)",
    "กดส่งข้อมูล — ระบบออกรหัสให้ทันที (ไม่ต้องรอเจ้าหน้าที่)",
    "ได้รับ QR code หน้าจอเดียวกัน — ใช้เข้าห้องสมุดได้ \"เฉพาะวันนี้\" เท่านั้น",
])
info_box("ตั้งแต่ กรกฎาคม 2569 เป็นต้นไป เลขบัตรประชาชนของรายวัน \"ไม่บังคับ\" แล้ว — บังคับเฉพาะชื่อ-สกุล "
         "เพื่อความสะดวก หากผู้ใช้ไม่กรอกเลขบัตร ระบบจะออกรหัสอ้างอิงภายในให้เอง ทั้งนี้กรณีไม่กรอกเลขบัตร "
         "ระบบจะไม่สามารถระบุตัวบุคคลซ้ำได้ (ระงับสิทธิ์รายบุคคล/จำกัดจำนวนต่อคนจึงทำไม่ได้)")

heading("2.2  ข้อความ error ที่ผู้ใช้อาจเจอ (ไว้ตอบคำถามหน้าเคาน์เตอร์)", 2)
simple_table(
    ["สาเหตุ", "ข้อความที่ผู้ใช้เห็น", "สิ่งที่เจ้าหน้าที่ควรทำ"],
    [
        ["กรอกเลขบัตรมาแต่ผิด/ไม่ผ่านการตรวจสอบ", "เลขบัตรประชาชนไม่ถูกต้อง กรุณาตรวจสอบแล้วกรอกใหม่",
         "เกิดเฉพาะเมื่อผู้ใช้กรอกเลขบัตร — ให้ตรวจอีกครั้ง (มักพิมพ์ผิด/สลับหลัก) หรือเว้นว่างไว้ก็ได้"],
...
heading("รายวันจำเป็นต้องกรอกเลขบัตรประชาชนไหม?", 2)
para("ไม่จำเป็น — บังคับเฉพาะชื่อ-นามสกุล ส่วนเลขบัตรจะกรอกหรือไม่ก็ได้ หากกรอกจะช่วยให้ระบบระบุตัวตน "
     "และป้องกันการใช้ซ้ำ/ระงับสิทธิ์รายบุคคลได้ แต่ถ้าไม่กรอกก็ยังออกรหัสให้ตามปกติ")
...
para("Smart Creative Learning Space — ระบบบุคคลภายนอกเข้าใช้บริการ  |  เวอร์ชัน 1.1  |  กรกฎาคม 2569",
     size=12, color=GRAY, align=WD_ALIGN_PARAGRAPH.CENTER)
