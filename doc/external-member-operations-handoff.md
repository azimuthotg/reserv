...
|---|---|---|
| กลุ่มเป้าหมาย | คนทั่วไป มาครั้งคราว | คนเข้าประจำ เช่น พนักงานส่งเอกสาร รวมถึงบุคคลสำคัญ/VVIP |
...
5. **บุคคลสำคัญ/VVIP (เช่น นายกสภามหาวิทยาลัย) — เลขบัตรไม่บังคับ (2026-07-10):**
   เจ้าหน้าที่ลงทะเบียนแทนที่ `/manage/external/register/` โดย**เว้นช่องเลขบัตรว่างได้**
   → api ออก **รหัสอ้างอิงขึ้นต้น `V` + เลข 12 หลัก** แทนเลขบัตร (รวม 13 ตัว ไม่ชนเลขบัตรจริง)
   จากนั้น flow เหมือนสมาชิกถาวรทุกอย่าง (กดอนุมัติ → ได้ permanent_code → บัตร+QR ที่หน้า detail
   เจ้าหน้าที่ส่ง/แสดง QR ให้เอง เพราะไม่มีเลขบัตรไว้กรอกดูบัตรที่หน้า public)
   หน้า list/detail แสดงเลขบัตรเป็น "—"/รหัสอ้างอิงสำหรับรายที่ไม่ได้ให้เลขบัตร
...
**ตอบคำถามทีมประตู "return อะไร เหมือนนักศึกษา/บุคลากรไหม" (2026-07-10):**
- **convention เดียวกัน: HTTP 200 = เปิดประตู** (เหมือน `/v2/student/`, `/v2/personnel/`) — traceon อ่านแค่ status ได้เลย
- body เป็น JSON ข้อมูลสมาชิกตามตัวอย่างข้างบน (field ไม่เท่ากับ นศ./บุคลากร เพราะเป็นคนละตาราง
  แต่มี `first_name`/`last_name` ให้แสดงชื่อได้; ไม่ต้อง parse ก็เปิดประตูได้)
- หมายเหตุ: สมาชิกถาวรที่ไม่ได้ให้เลขบัตร (บุคคลสำคัญ/VVIP) `member.citizen_id` จะเป็นรหัสอ้างอิง
  ขึ้นต้น `V` เช่น `V123456789012` — ไม่กระทบ logic ประตู (ดูแค่ HTTP 200 เหมือนเดิม)
