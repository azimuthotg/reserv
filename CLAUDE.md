...
progress: 99
phase: ระบบใช้งานจริง (production) ครบ 4 phase แล้ว — external access ปิดครบวงจร · บริการ VM 3 ห้อง (Canva Pro 1/2, ChatGPT) เปิดจอง 24 ชม. **ปุ่มเข้าใช้งานเชื่อม VM Gateway ครบวงจรแล้ว** พร้อมคู่มือผู้ใช้ · วันหยุดอัตโนมัติแบบเจ้าหน้าที่อนุมัติ + เตือนข้อมูลค้างใช้งานจริงแล้ว · เหลือ LINE Rich Menu (ผู้ใช้ทำเอง) + รอบัญชี Canva พร้อมจึงประกาศคู่มือ + งาน enhancement
done_2026-07-10:
...
done_2026-08-09:
  ...
  - ✅ **หน้า `/manage/holidays/` เรียงแบบไทม์ไลน์** — ที่ยังไม่ถึงอยู่บน (ใกล้สุดก่อน) · ที่ผ่านมาแล้วอยู่ล่างและจางลง มีเส้นคั่น · **ไฮไลต์ "หยุดถัดไป" พร้อมนับถอยหลัง** (นับเฉพาะรายการที่เปิดใช้แล้ว เพราะฉบับร่างยังไม่บล็อกการจอง) · บอกด้วยถ้าวันหยุดถัดไปอยู่คนละปีกับแท็บที่เปิดอยู่ · test 56/56 ผ่าน (เพิ่มใหม่ 5 เคส)
  - ✅ **วันหยุดอัตโนมัติ (งานค้างตั้งแต่ 12 ก.ค.)** — พบว่าตารางวันหยุด**หยุดอัปเดตตั้งแต่ 3 มิ.ย.** ทำให้ **12 ส.ค. (วันแม่) มีคนจอง MINI THEATER ไปแล้ว 2 รายการ** (เจ้าหน้าที่ยกเลิก+แจ้งนักศึกษาแล้ว) · เพิ่ม `booking/holiday_feed.py` ดึง iCal วันหยุดไทยจาก Google Calendar + command `sync_holidays` + ปุ่มดึงในหน้า `/manage/holidays/` + **แถบเตือนสีแดงบนแดชบอร์ด** เมื่อมีวันหยุดใกล้ถึงที่ยังไม่ตรวจ (พร้อมจำนวนการจองที่ตกวันนั้น) · **ดึงเข้ามาเป็นฉบับร่าง `is_active=False` เสมอ** เพราะฟีดผิดได้ 2 ทาง (มีวาเลนไทน์/ตรุษจีน/คริสต์มาสปนมา · ขาดวันเข้าพรรษา) · เพิ่ม `HolidayDate.source` (migration `0016`) กัน sync ทับแถวที่เจ้าหน้าที่กรอกเอง · **test 50/50 ผ่าน** (เพิ่มใหม่ 12 เคส) · ดึงจริงแล้ว 35 วัน (2569–2570) และเปิดใช้ 12 ส.ค. แล้ว
  - ✅ **รูปห้อง `chat-gpt.png` ใหม่** — เข้าชุดกับ Canva Pro 1/2 (โทนเขียวมรกตบนพื้นเข้ม มีชื่อพาด) · **1440×810** 703 KB (พื้นเข้มมี particle เยอะ ที่ 1920 จะได้ 1,281 KB หนักกว่าใบอื่นเท่าตัว) · เพิ่มสคริปต์ใช้ซ้ำ [doc/prep_room_image.py](doc/prep_room_image.py) พร้อมสเปกรูปห้องใน MEM.md
...
done_2026-08-10:
  - ✅ **deploy `cb557c5` (หน้าวันหยุดแบบไทม์ไลน์) ขึ้น prod + ผู้ใช้ทดสอบผ่านแล้ว**
  - ✅ **ปุ่ม "เข้าใช้งาน" ของห้องออนไลน์ → VM Gateway** (ปิดงานค้างตั้งแต่ 8 ส.ค.) — ทีม VM ยืนยัน URL `https://arcvm.npu.ac.th/vm/login/` **ตรวจกับของจริงแล้ว** (200 · TLS ผ่าน · URL เดิม redirect มาตัวใหม่) · `/api/my-bookings/` ส่ง `is_online` เพิ่ม · ห้องออนไลน์ได้ปุ่มเปิดแท็บใหม่ ห้องจริงยังได้ปุ่มควบคุมอุปกรณ์ · ปุ่มขึ้น 5 นาทีก่อนเวลาจนสิ้นสุด · URL/นาทีอยู่ใน `.env` มี test กัน hardcode และกันถอยกลับไปใช้ http · แก้ `how_to_use`+`rules` ของ 3 ห้องผ่าน [doc/prep_vm_rooms_gateway.py](doc/prep_vm_rooms_gateway.py) · **deploy + เทส prod ผ่าน (ปุ่มขึ้นจริงตอน 22:55 สำหรับการจอง 23:00)** · test 77/77
  - ✅ **🔴 พบ+แก้บั๊ก production: คอมเมนต์นักพัฒนาหลุดขึ้นหน้า `/room/<key>/` ทุกห้อง** — `{# #}` ของ Django ใช้ได้บรรทัดเดียว เขียนคร่อม 2 บรรทัดจะพ่นออกมาเป็นข้อความให้ผู้ใช้เห็น ติดมาตั้งแต่ `8f39c2a` (9 ส.ค.) · เจอตอนแคปหน้าจอทำคู่มือ · เปลี่ยนเป็น `{% comment %}` + เพิ่ม `TemplateCommentTests` สแกนทุกไฟล์ .html กันซ้ำ · **deploy + ตรวจ prod ผ่าน**
  - ✅ **ส่งหนังสือแจ้งทีม VM ว่าผูกปุ่มเสร็จแล้ว** (ข้อ 10.5 ของหนังสือ 10 ส.ค.) — ทีม VM ปิดช่องทางเดิม `http://202.29.55.180:8888` ได้
  - ✅ **คู่มือการจองบริการเครื่องเสมือน** [doc/vm-booking-guide-2569.docx](doc/vm-booking-guide-2569.docx) 5 บท 12 ภาพจาก production จริง — สร้างซ้ำได้ด้วย [doc/capture_vm_booking_flow.py](doc/capture_vm_booking_flow.py) (จองจริงแล้วยกเลิกให้เอง) + [doc/make_vm_booking_guide_docx.py](doc/make_vm_booking_guide_docx.py) · **ยังไม่ประกาศ** รอบัญชี Canva พร้อม
  - ✅ **ปิดช่องโหว่ "แดชบอร์ดเงียบเพราะไม่มีข้อมูล"** — แถบเตือนฉบับร่างเดิมขึ้นได้เฉพาะเมื่อ *มีแถวฉบับร่างอยู่จริง* แดชบอร์ดที่เงียบเพราะตรวจครบจึงหน้าตาเหมือนกันเป๊ะกับที่เงียบเพราะไม่มีใครกดดึงมาหลายเดือน (รูปแบบเดียวกับตอนตารางค้างจนมีคนจอง 12 ส.ค. ได้) · เพิ่ม model `HolidaySyncRun` (migration `0017` + data migration bootstrap กันเตือนผิดตัววันแรก) บันทึกทุกครั้งที่ดึงสำเร็จ **แม้ไม่มีวันใหม่เลย** · `data_status()` เป็นค่ากลางให้แดชบอร์ด + `/manage/holidays/` เตือน 2 กรณี: ไม่ได้ดึงเกิน 45 วัน / วันหยุดในตารางครอบไปข้างหน้าไม่ถึง 60 วัน · ขึ้น Django Admin แบบอ่านอย่างเดียว · **test 70/70 ผ่าน** (เพิ่มใหม่ 14 เคส) · **deploy prod (migration `0017`) + ตรวจผ่านแล้ว — แดชบอร์ดเงียบตามที่ควรเป็น ยืนยัน bootstrap ทำงานถูก**
done_2026-08-13:
  - ✅ **🔴 พบ+แก้บั๊ก production: โปรไฟล์ขึ้น "รหัสถูก แต่ชื่อผิด"** — `/api/check-user/` คืน `userLdap` สดจาก api คู่กับ `full_name` จาก cache 30 วันของ `booking_lineuser` ซึ่งผูกกับ **LINE userId ไม่ใช่รหัส** เจ้าหน้าที่ลบการผูกฝั่ง api แล้วให้ผูกใหม่จึงไม่หาย เพราะ fast path เช็คแค่ความสดของ cache ไม่ได้เช็คว่ารหัสเปลี่ยน · แก้ `_get_or_refresh_line_user()` ให้ยกเลิก fast path เมื่อรหัส/ประเภทไม่ตรง + รับ `force=` (หน้า register ใช้เสมอ) + api ล่มต้องไม่ล้างชื่อเดิมเป็นค่าว่าง · `sessionStorage` ทั้ง 3 หน้ามีอายุ 30 นาที · เพิ่มคำสั่ง `refresh_profile` แก้รายคน · **test 82/82 ผ่าน (เพิ่มใหม่ 5 เคส) · deploy prod แล้ว**
  - ✅ แก้ข้อมูลผู้ใช้ที่แจ้งเข้ามาให้ถูกต้องแล้ว 1 ราย และตรวจว่าการจองที่มีอยู่ไม่กระทบ
  - ✅ สแกนทั้งฐาน: รหัสที่ผูกมากกว่า 1 LINE มี 15 รหัส แต่ **ใช้จองจริงจากมากกว่า 1 LINE = 0 เคส** จึงยังไม่มีใครได้โควตาเกิน (ดู MEM.md)
next:
  - **[รอทีม VM แจ้งกลับ] ประกาศคู่มือ [doc/vm-booking-guide-2569.docx](doc/vm-booking-guide-2569.docx) ให้ผู้ใช้ทั่วไป** — ทีม VM ขอให้รอจนติดตั้งบัญชี Canva เสร็จ (ข้อ 8) · เมื่อพร้อมแล้วให้**ลบย่อหน้าหมายเหตุในหัวข้อ 1.1** ของ `doc/make_vm_booking_guide_docx.py` แล้วสร้างเอกสารใหม่
  ...
  - **[ไม่บังคับ — ผู้ใช้ปฏิเสธแล้ว 2026-08-10 ไม่อยากสร้างภาระให้ prod] ตั้ง Task Scheduler รัน `deploy/sync_holidays.bat` เดือนละครั้ง** · คำสั่ง `schtasks` เตรียมไว้ใน [doc/deploy_guide.md](doc/deploy_guide.md) แล้ว
risks:
  ...
  - ตารางวันหยุดเคยค้างไม่อัปเดต 2 เดือนจนนักศึกษาจองวันหยุดได้ — ปิดช่องแล้วด้วย `HolidaySyncRun` (แดชบอร์ดเตือนเมื่อไม่ได้ดึงเกิน 45 วัน) **แต่ยังพึ่งคนกดปุ่มอยู่ ไม่มีตัวรันอัตโนมัติ** (ดู MEM.md)
  - โควตาและกฎห้ามจองซ้อนนับต่อ **LINE account** ไม่ใช่ต่อรหัสนักศึกษา → ผูกหลาย LINE = ได้โควตาหลายเท่า (ผู้ใช้ตัดสินใจ 2026-08-13 ยังไม่แก้ เพราะสแกนแล้ว 0 เคส — ดู MEM.md)
updated: 2026-08-13
-->
...
**ปุ่มเข้าใช้งานของห้องออนไลน์ (VM Gateway):** การ์ดการจองในหน้าแรกเลือกปุ่มจาก `is_online`
ที่ `/api/my-bookings/` ส่งมา — ห้องออนไลน์ได้ปุ่ม **"🖥️ เข้าใช้งาน"** เปิดแท็บใหม่ไป
`settings.VM_GATEWAY_URL` ส่วนห้องจริงได้ปุ่ม "🎛️ ควบคุมอุปกรณ์" ไปหน้า IoT ตามเดิม
- **URL เดียวใช้ได้ทุกห้อง** — Gateway ตรวจการจองจากบัญชีที่ล็อกอินแล้วพาเข้าเครื่องเอง
  ไม่ต้องส่งรหัสการจองหรือพารามิเตอร์ใด ๆ · **ห้ามลิงก์ไป Guacamole ตรง ๆ** (ข้ามการตรวจสิทธิ์)
- ปุ่มขึ้นตั้งแต่ `VM_GATEWAY_EARLY_MINUTES` (5) นาทีก่อนเวลาจน สิ้นสุดเวลาจอง — ต้องตรงกับฝั่ง Gateway
- `target="_blank"` เสมอ เพราะเป็นการควบคุมหน้าจอระยะไกล หน้าต่างในแอป LINE ใช้ได้ไม่ครบ
- ค่าทั้งสองอยู่ใน `.env` (`VM_GATEWAY_URL`, `VM_GATEWAY_EARLY_MINUTES`) **ห้าม hardcode ในเทมเพลต**
